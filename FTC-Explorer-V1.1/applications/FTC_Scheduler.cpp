...
{	
	...
	imu.updateSensor();
		...
		{		
			float tempacc=pythagorous3(imu.Acc.x, imu.Acc.y, 4096-imu.Acc.z);
			tempacc=sqrtf(tempacc);
			if(tempacc>maxacc) maxacc=tempacc;	
			if(maxacc>100) start=true;
		}	
	//计算飞行器姿态
	imu.getAttitude();
	
	//飞行器姿态内环控制
	if((rc.rawData[AUX2]>0 && rc.rawData[AUX2]<1300)&&(rc.rawData[AUX1]>0 && rc.rawData[AUX1]<1300))
		{
			t=0;start=false;
		}
		...
		fc.Attitude_Inner_Loop(t,maxacc,start);
	//飞行器高度传感器数据更新
	nav.update();
}
